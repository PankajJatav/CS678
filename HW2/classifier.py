...
from AdamOptimizerAsPaper import CustAdam
...
def train(args):
    ...
    optimizer = AdamW(model.parameters(), lr=lr, betas=(b1, b2), eps=eps)
    # print(model)
    if args.use_paper_adam:
        print("Inside CustAdam", args.warm_up, b1, b2, eps)
        optimizer = CustAdam(model.bert.config.vocab_size, 1, args.warm_up, AdamW(model.parameters(), lr=0,  betas=(b1, b2), eps=eps))
...
def get_args():
    ...
    parser.add_argument("--use_paper_adam", action='store_true')
