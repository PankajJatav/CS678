...
class SentimentExample:
    ...
    def __init__(self, words, label, word_indices = []):
        self.words = words
        self.label = label
        self.word_indices = word_indices # the word indices in vocab

    def __repr__(self):
        return repr(self.words) + "; label=" + repr(self.label) + "; word_indices " + repr(self.word_indices)
...
def indexing_sentiment_examples(exs: List[SentimentExample], vocabulary: List[str], UNK_idx: int):
    ...
    """
    for ex in exs:
        ex.set_word_indices([int(vocabulary.index(word)) if word in vocabulary else UNK_idx for word in ex.words])
...
class SentimentExampleBatchIterator:
    ...
    def get_next_batch(self):
        if self._cur_idx < len(self.data): # loop over the dataset
            ...
            batch_lengths = np.asarray([len(x.words) for x in batch_exs ])
            batch_labels = np.asarray([ x.label for x in batch_exs ])
            max_len = max(batch_lengths)
            batch_inputs = np.asarray([ np.pad(x.word_indices, pad_width=(0, max_len - len(x.word_indices) ), mode='constant') for x in batch_exs])
