...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    p_attention = torch.matmul(query, key.transpose(-1, -2))

    p_attention_scores = p_attention / math.sqrt(self.attention_head_size)
    p_attention_scores = p_attention_scores + attention_mask

    attn_prob = F.softmax(p_attention_scores, dim=-1)
    attn_prob = self.dropout(attn_prob)

    attn_value = torch.matmul(attn_prob, value)
    attn_value = attn_value.transpose(1, 2).contiguous()
    attn_value = attn_value.view(attn_value.shape[0], attn_value.shape[1], self.all_head_size)
    return attn_value
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    sublayer = dense_layer(output)
    return dropout(ln_layer(input + sublayer))
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    added_attention = self.add_norm(hidden_states, attention, self.attention_dense, self.attention_dropout,
                                    self.attention_layer_norm)

    # feed forward
    interm_feed = self.interm_dense(added_attention)
    interm_output = self.interm_af(interm_feed)

    # another add-norm layer
    final_output = self.add_norm(added_attention, interm_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    return final_output
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    pos_embeds = self.pos_embedding(pos_ids) #None
